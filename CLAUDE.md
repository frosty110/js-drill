...
| `css/15-ds-shell.css` · `css/16-sd-shell.css` | The shell's own styles (header bar, scope meter, due chip, nav-as-links) and the system-design page's integration with them (fitting a fixed rail / bottom bar around its sticky header and centred main) — the sibling of `css/06-ds-nav.css`, which does that job for `index.html`. |
...
| `tools/cdp/nav-hierarchy.js` | Durable probe for the hierarchy — a breadcrumb on every non-root surface and none at a root, trail depth matching the registry, the leaf unlinked and `aria-current`, ancestors climbing exactly one level, lessons/units named by title rather than id, no horizontal scroll at 390px. 24 assertions across both pages and both breakpoints. |
