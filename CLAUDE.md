...
| `data/system-design/components/catalog.json` | **The component catalog** — the Building Blocks topic INVERTED. 66 components in 11 categories, each carrying `what` (mental model), `reachFor` / `avoid` (the *signal in a prompt*, not properties — this is the selection surface), `costs`, `failureModes`, `alternatives[]` (sibling + what decides between them), an optional `mechanism` linking it to the `tags.json` facet, and an optional `drill` pointer at an existing question set. Every authored mechanism is covered, and all 66 components carry problem edges. An edge exists where the component is a DECISION in that problem, not merely present. Contract + authoring rules: [`docs/component-catalog.md`](docs/component-catalog.md). |
| `data/system-design/mechanism-map.json` | **The edges of the component↔problem graph** — 262 annotations over all 66 components (5–12 per problem), keyed component → design-problem, each saying what that component is *doing in that problem* ("the per-user timeline of post IDs, hydrated on read"). Written once, rendered at BOTH endpoints, so an annotation must be a predicate about the job rather than a sentence that only reads correctly from one side. Without it a component page is a list of problem names — a link farm. The validator's coverage gate makes it impossible to tag a problem with a mechanism and forget the annotation. |
...
| `tools/check-boot-weight.js` | Gate: per-page ceiling on the bytes a phone fetches at boot, **plus a ceiling on the service worker's `APP_SHELL`**. `--report` prints the breakdown. Mermaid (3.4 MB) loads on first diagram render and is runtime-cached rather than precached — it is only used by `system-design.html`, while only `index.html` registers the worker. |
