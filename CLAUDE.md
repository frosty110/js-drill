...
| `tools/build-component-cards.js` | **Generates the flash-card decks** from `components/catalog.json` + `mechanism-map.json` — five templates (selection · rejection · failure · discrimination · transfer) emitted as `open` questions into the `component-cards` topic. Committed generated output, gated by `--check` in `tools/check-all.js`. Holds two rules: an **order ledger** (`component-cards/order.lock.json`) giving every card a permanent slot, because SR keys and share codes are positional and a regenerated deck must never reshuffle; and **no padded rubrics** — a card the source cannot supply 3 real points for is reported, not invented. |
| `data/system-design/component-cards/` | 11 generated decks (one per catalog category, 308 cards) plus `order.lock.json`. **Do not hand-edit** — regenerate. |
| `tools/cdp/sd-component-cards.js` | Durable probe for the decks — topic registered with an svg mark, every deck renders, a card is an open/reveal/self-grade flash card whose reveal carries both the rubric points and the prose answer, grading advances and writes Leitner state, the component page's CTA reaches its OWN category's deck, and the order ledger matches the emitted decks. 41 assertions. |
