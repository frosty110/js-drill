...
| `tools/cdp/sd-bretton-arc.js` | Durable probe for the **API** and **architecture + data model** steps of the canonical arc on `p33`–`p36`. Two silent failures live here. A malformed Mermaid diagram does **not** throw — `renderDiagramInto` catches it and falls through to the same `<pre class="diagram-src">` used when the CDN is blocked, so the validator (which only checks the header line) stays green while the deck renders a wall of source. And `afterQuestion` is a POSITIONAL index, so inserting a question shifts every anchor after it onto a still-valid but *wrong* question — nothing errors, the deck just teaches the wrong thing. Asserts each new question's diagram reaches a real `<svg>`, that the data-model question carries its own diagram **plus** the re-anchored architecture overview, that the rubrics name real `/v1/` endpoints and real `table(col, …)` definitions rather than prose about having them, and no 390px overflow. 60 assertions. |
