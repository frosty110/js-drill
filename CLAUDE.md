...
| `tools/check-icons.js` | Gate for invariant 9. Three checks: every referenced icon NAME resolves against `DS_ICONS` (a typo makes `dsIcon` return `''` — an icon that silently isn't there), no inline `<svg>` outside `ds/icons.js`, and no emoji in chrome. The emoji half is a **ratchet**: design-system paths hold zero and a new file is strict by default; the pre-system surfaces carry a per-file budget in `data/icon-debt.lock.json` that may only fall. `--accept` re-baselines the legacy budget only — a strict path can never be re-baselined, which is what makes it one-way. |
| `data/icon-debt.lock.json` | The legacy emoji-in-chrome budget. Generated by `tools/check-icons.js --accept`; a file that reaches zero drops out of the lock and is strict from then on. |
