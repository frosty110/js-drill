...
| `js/app/01..24-*.js` | Main app logic, split from the old `app.js` monolith into ordered slices that share global scope. Load order matters. Named by concern. **`14-init-core.js` was itself re-split into `14a..14g`** (`tools/split-init-core.py`): it was the residue of the 2026-05 cut-by-size refactor and had grown to 2,502 lines holding 24 unrelated concerns — palette, bootstrap, 25 drill launchers, settings toggles, PWA install, At-Risk, Streak Map, an audio player, Heatstrip, Stats, Dashboard, Today's Plan, backup/restore, cheatsheet, boot tail. Cut along its own banners; largest slice 481 lines. Gated by `--check` (order **and** service-worker precache coverage). |
...
| `system-design.html` | The standalone **concept-recall drill** (no code execution) — System Design *and*, since 2026-08-05, the **AI books shelf** — **markup + inline CSS only (~565 lines)**; its ~1,950 lines of application logic live in `js/sd/01..15-*.js` (see below). **Multi-topic, two shelves**: a topic landing (grouped by `shelf`) → per-topic chapter list → drill. *System Design shelf*: **DDIA** (12 ch — 1st-edition chapter structure, with the 2nd edition's additions folded into the chapter each belongs to; see the topic manifest's `description`), **The System Design Interview** (framework + estimation + a two-unit *Designing with Agents* part, 9 §), **Building Blocks** (components, 7 §), **Canonical Design Problems** (32 worked problems). *AI shelf*: **AI Engineering** (Chip Huyen, 10 ch), **Agentic Design Patterns** (Antonio Gulli, 21 patterns), **Designing Multi-Agent Systems** (Victor Dibia, 15 ch) — same engine, same Leitner store, no infographics or tags (those stay opt-in per topic). Two question types: **MC** and **open** ("explain & apply" — reveal model answer + self-grade). A topic manifest can set `ordered:true` (design problems) so its units drill in authored arc order instead of due-first shuffle. Tapping a unit opens a **detail screen** surfacing the authored `keyTakeaways`, followed by a visual study set and drill controls. `data/system-design/infographic-plan.json` plans distinct graphics per lesson; `infographic-sets.json` authors each sheet's description, numbered flow, numbers, priorities, and trade-offs before pixels. All 51 eligible lessons use multi-image sets (183 registered sheets; 123 PNGs committed and 60 awaiting artwork). `drill-infographic-set` renders the ordered lesson set; `drill-infographic` and one shared workspace provide full-screen Fit/100%/zoom/pan/pinch/download. **Diagrams**: content may use legacy singular `diagram` or a `diagrams[]` visual deck. Each deck item is `{id,title,kind,role,takeaway,afterQuestion,code}`. `afterQuestion` attaches each focused visual to the relevant revealed answer. Every canonical design problem deck contains four architecture diagrams (overview, signature mechanism, scale/path, failure/consistency); focused question-level `diagram` request flows remain supported. Mermaid loads from CDN and degrades gracefully. Reuses `ds/tokens.css` + `ds/components.css` + `js/storage.js`; Leitner SR under `jsdrill.systemdesign.v1`. Design problems are grouped into **7 transferable-mechanism families** with a faceted **Filter** panel (see `data/system-design/tags.json`); chapter ids are permanent while display order comes from `parts[]` via `displayNum`. The topic hero reports **unit** mastery ("N of 32 problems") with the question count as a labeled sub-line. A **study-plan strip** below the hero offers time-budgeted routes through the problems (see `data/system-design/plans.json`) in two labeled rows — authored budgets, then the tag-derived company loops. Deep-linkable: `#/<topic>`, `#/<topic>/<unit>`, `#/<topic>/mixed`, `#/<topic>/tag/<facet>/<value>`, `#/<topic>/plan/<planId>` (company plans are the two-segment `plan/company/<name>`), plus the catalog pair `#/components/catalog` and `#/components/c/<componentId>`. **Component catalog**: the Building Blocks topic also renders inverted — one page per building block (when to reach for it, what it costs, how it breaks, which sibling to rule out) cross-linked to every canonical design problem that uses it, with each edge annotated. The traversal runs both ways: a problem lists its components, a component lists its problems, and both show the same annotation. See [`docs/component-catalog.md`](docs/component-catalog.md). **Interleaved-session context**: a mixed/due session shuffles across units, so every card there — MC *and* open — leads with a context block (unit label + the authored `summary`); chapter and plan sessions omit it because `session.title` already names the unit. Under the stem, a unit's authored `brief` renders as a collapsed *Requirements & scale* disclosure (retrieval first, numbers on one tap), suppressed on the `brief.gate` questions whose own answer IS the brief and re-shown beside the model answer there. |
...
| `data/system-design/topics.json` | Registry of concept-drill topics (`id`, `icon`, `title`, `kind`, `blurb`, `shelf`) that the landing page enumerates, plus a `shelves[]` list (`id`, `title`, `blurb`). Add a topic by appending here + creating its `<id>/` dir. **`shelf` groups topics by subject** — `system-design` (DDIA · method · building blocks · design problems) and `ai` (the three AI books, 2026-08-05). The landing page renders one heading per shelf and Home renders one track card per shelf, so a book added to the AI shelf surfaces on both pages with its own denominator. A topic with no `shelf` counts as System Design, so an older registry renders unchanged. |
| `data/system-design/<topic>/` | Per-topic content — `manifest.json` (`parts[]` + `chapters[]`, with optional `unitLabel`/`unitAbbrev`) + per-unit files (`chNN`/`sNN`/`cNN`.json). Each unit's `questions[]` mixes `type:"mc"` (4 options + `answer` index + `explain`) and `type:"open"` (`prompt` + `points[]` rubric + model `answer`). A unit may also carry `brief` — `{functional[], scale[], gate[]}`, the scoped requirements and scale constants an interleaved session can't reconstruct (REQUIRED on `design-problems`, whose prompts mostly never name their own system). `ddia/AUTHORING-BRIEF.md` + `OPEN-QUESTION-BRIEF.md` + `design-problems/BRIEF.md` document the schema/quality bar. Gated by `tools/validate-system-design.js`. |
| `data/system-design/tags.json` | **Faceted tag registry for Canonical Design Problems** — the system-design sibling of `data/tags.json`. 5 facets: `mechanism` + `difficulty` + `company` (AUTHORED on manifest chapter entries under `tags`, so filtering works before any unit file is fetched) and `family` + `length` (DERIVED from the part name and the `questions` count — no authoring). AND-across / OR-within, same as the main app's Problems filter. Registry-closed: the validator rejects an authored value that isn't listed here, which is what stops near-synonyms accumulating. `mechanism` is the cross-family transfer index — every problem carries 2–4. Those tags are also the EDGE SET of the component catalog: each problem's tagged mechanisms are its **signature** components, sorted first and marked inside the unit-detail screen's **Components in play** block (component + what it is doing *in this problem*), which replaced the bare chips. That block is derived from the EDGE FILE, not from these tags — the facet stays at 32 headline values because it is the filter index and 60 chips would be unusable on a phone. The `#/design-problems/tag/mechanism/<value>` route still exists and is reachable from the component page. See [`docs/component-catalog.md`](docs/component-catalog.md). |
| `data/system-design/components/catalog.json` | **The component catalog** — the Building Blocks topic INVERTED. 65 components in 11 categories, each carrying `what` (mental model), `reachFor` / `avoid` (the *signal in a prompt*, not properties — this is the selection surface), `costs`, `failureModes`, `alternatives[]` (sibling + what decides between them), an optional `mechanism` linking it to the `tags.json` facet, and an optional `drill` pointer at an existing question set. Every authored mechanism is covered, and all 65 components carry problem edges. An edge exists where the component is a DECISION in that problem, not merely present. Contract + authoring rules: [`docs/component-catalog.md`](docs/component-catalog.md). |
| `data/system-design/mechanism-map.json` | **The edges of the component↔problem graph** — 253 annotations over all 65 components (5–12 per problem), keyed component → design-problem, each saying what that component is *doing in that problem* ("the per-user timeline of post IDs, hydrated on read"). Written once, rendered at BOTH endpoints, so an annotation must be a predicate about the job rather than a sentence that only reads correctly from one side. Without it a component page is a list of problem names — a link farm. The validator's coverage gate makes it impossible to tag a problem with a mechanism and forget the annotation. |
| `tools/cdp/sd-component-catalog.js` | Durable probe for the catalog — entry point, category grouping, most-used-first ordering with authored-order tiebreak, the five decision blocks, the annotated Used-in list, **the same-annotation-at-both-endpoints invariant** (the one whose failure is invisible on both pages), round-trip traversal, signature-vs-supporting ordering, the surfaced tagging (signature marks, the persisted role filter, the detail chip row), no-orphans, deep links, degradation for an unknown component and for a topic with no catalog, mobile+desktop geometry. 51 assertions. |
| `tools/cdp/sd-component-return.js` | Durable probe for **getting back out** of a component page — the header following an in-page navigation, the breadcrumb having non-zero width and a readable up-link at 390px, the return row naming the problem you came from (and surviving an *Instead, consider* hop and a browser Back), a deep link and a catalog entry claiming no origin, and the desktop trail staying full. Every navigation in it is a real **click**: `setRoute` writes the URL with pushState, so a probe that sets `location.hash` fires the hashchange that repaints and passes against the broken build — which is exactly how the stale-header bug survived every other probe. 37 assertions. |
...
| `tools/cdp/sd-mixed-context.js` | Durable probe for interleaved-session context — every mixed/due card (open **and** MC, across a whole session) leads with its unit label + authored scope, the label names a real unit, chapter sessions omit the block, and the `brief` disclosure renders collapsed, opens on tap, honours `brief.gate`, and clears the 44px tap-target floor at mobile + desktop. 19 assertions. |
| `tools/cdp/sd-tags-nav.js` | Durable probe for the design-problems taxonomy + faceted filter — 7 mechanism families, unit-level hero rollup, contiguous `displayNum` over non-sequential ids, chip rows, AND/OR filter semantics, empty state, tag deep links, other-topic isolation, unclipped card titles at 390px, mobile+desktop overflow. Resets `jsdrill.systemdesign.v1` first — the tag deep-link cases persist a filter, so without it a second consecutive run booted filtered and failed on a chip that was no longer on screen. 42 assertions. |
...
| `ds/components.css` | **Single source of truth** for reusable UI primitives — page frame (`.ds-page`/`__head`/`.ds-section`), button, card, chip, row, stat, field, MC option, switch, segmented, progress, **session HUD (`.ds-hud`)**, sheet/scrim, adaptive nav, empty state, skeleton. Built only from `ds/tokens.css`; contains no app selectors (D04). **Composing it is invariant 10, gated by `tools/check-ds-adoption.js`** — a page may say where a component sits, not what it looks like. `.ds-hud` is the worked example of when to ADD a primitive here: both pages had already built the same session strip (`#review-hud` / `.plan-hud`), and the sd one carried a comment saying it matched the app's directly above a from-scratch reimplementation. Two independent builds of one thing = a missing primitive, not a page that ignored one. |
| `ds/dragscroll.js` | **Click-and-drag horizontal scrolling**, loaded by all three pages. Horizontally-overflowing strips (tab rows, chip rows, the study-plan cards) are swipeable on a phone for free, but a desktop mouse has no way to reach content past the right edge — the scrollbars are hidden by design and shift+wheel isn't discoverable. ONE delegated pointer handler finds the nearest horizontally-scrollable ancestor of whatever you pressed, so every overflowing region gets the affordance without being wired up, including ones added later. Mouse only (touch already drags natively); text surfaces (`pre`, `code`, inputs, CodeMirror) are exempt so drag-selecting code to copy still works; a drag past 4px swallows the click that follows it, so dragging a strip of cards can't also open one. Probed by `tools/cdp/ds-dragscroll.js`. |
| `ds/icons.js` | **The only icon vocabulary** — `dsIcon(name, px)` + `DS_MODE_ICONS`. 24×24 stroke glyphs; every one is stamped `.ds-icon` so alignment is the set's job, not the call site's. Emoji is banned from chrome and inline `<svg>` is banned everywhere else (D07 / invariant 9). Add new glyphs here. Gated by `tools/check-icons.js`. |
| `css/17-sd-page.css` | `system-design.html`'s page CSS. It was **inline in the HTML** until 2026-08-05 — ~420 lines that no file could `@import`, no gate could read and no reviewer comparing the two pages would open, which is how that page came to carry its own card (×4), chip (×8), progress bar (×4), stat tile, session HUD and three empty states while `ds/components.css` already had all of them. Now extracted and converted: the primitives come from `ds/`, and what stays here is layout and the handful of things the design system has no opinion about (a filter chip that is a button, a callout's left rule, the hero's gradient). Re-declared primitives went 23 → 7, held there by `tools/check-ds-adoption.js`. |
| `tools/check-ds-adoption.js` | Gate for invariant 10 — the design system is USED, not just present. A **ratchet**, not a flat zero: `data/ds-adoption.lock.json` records per-file counts of re-declared primitives (a rule that paints its own surface + border + radius, or its own pill, or its own progress track) and raw hex, and those may fall, never rise. It counts DECLARATIONS not selector names, so a `.thing-card` setting only `display:flex` is composing correctly. `--accept` re-baselines and is right only when the number went DOWN; a number that must go UP means the answer is a new primitive in `ds/components.css`, not a new one in a page file. The legacy `css/01`–`05` carry most of the remaining budget and are deliberately not rewritten — the point is that they cannot grow. |
| `tools/check-icons.js` | Gate for invariant 9, flat zero, no escape hatch. Five checks: every referenced icon NAME resolves against `DS_ICONS` (a typo makes `dsIcon` return `''` — an icon that silently isn't there); every launchable mode has a `DS_MODE_ICONS` row (a missing one puts the label's first LETTER in a row of icons); no emoji in chrome; no text glyph (`▸ › ✓ ✕`) doing an icon's job; no inline `<svg>` outside `ds/icons.js`. It shipped as a ratchet against 492 legacy glyphs; that backlog is cleared, so the budget/lock/`--accept` are gone. |
| `tools/cdp/ai-shelf.js` | Durable probe for the AI-books shelf — the landing page's shelf headings and ordering, all seven topic cards, deep links into each of the three AI topics, and Home rendering the AI card **separately from System Design with its own denominator** (the failure that would otherwise report AI progress as System Design progress). 14 assertions. Registered in `check-all.js --probes`. |
| `docs/iconography.md` | **The icon rules** — the one rule, the icon-vs-typography line (and chevron-vs-arrow), sizes, tiles, how to add a glyph, `DS_MODE_ICONS`, the `data-icon` boot convention, and what each of the five gate checks catches. |
| `tools/cdp/sd-icons.js` | Durable probe for the rendered result — the four System Design topic marks are svgs in one tile size, every glyph on the page carries `.ds-icon`, no surface renders an emoji, the filter caret flips, the answer verdict and per-question link are icons, and **the wordmark is byte-identical to the mark the app's nav rail draws for Design** (the cross-page invariant no single-file check can see). 24 assertions per viewport, mobile + desktop. |
| `ds/gallery.html` | Visual catalog of every primitive in both themes, plus the **icon sheet** — all 89 glyphs rendered by name, GENERATED from `DS_ICONS` so it can't fall behind, with an accent border on any a mode claims. Open it before building anything new, and before adding an icon. |
...
| `docs/information-architecture.md` | **What surfaces exist and why** (D15) — the four questions the app answers, the settled vocabulary (track/section/lesson/scope/destination/session/drill), the header-as-progress-instrument contract, the nav closed at **three** destinations, `parent` on `js/routes.js` as the one mechanism behind breadcrumbs + up-nav + `aria-current` + scoped progress, a disposition row for every current surface, and a 7-phase migration. **Decided, not built** — read it before adding a surface, so the loops stop re-adding ones that already exist. Sibling to `ui-ux-guide.md` (how a surface looks) and `url-contract.md` (how it's addressed). |
| `tools/cdp/ds-dragscroll.js` | Durable probe for `ds/dragscroll.js` — a real CDP mouse drag scrolls the strip, does NOT fire the click underneath, a sub-threshold press still activates the card, `<pre>` keeps its drag-select, and the document itself never drag-scrolls. Resets the topic's localStorage first so consecutive runs test the same screen. 13 assertions. |
...
| `js/routes.js` | **The addressable-surface registry** — one row per PLACE the app can put the user, each declaring `content` or `action`. `parseAppHash()` inverts `appHash()`, so `system-design.html`'s router is an adapter over this table rather than a second parser. Governed by [`docs/url-contract.md`](docs/url-contract.md), invariant 7 — one row per surface (static share path, live-app hash, path parser, sitemap flag). Share URLs, backlinks, parsing and `sitemap.xml` all derive from it, so adding a crawlable surface is a one-line change. Each row also declares **`parent`** (D15 phase 1) — the surface that CONTAINS it — from which `ancestors()`/`crumbs()` derive the breadcrumb, and in later phases the single up-affordance, truthful `aria-current` and the header's scoped progress. |
| `js/sd/01..15-*.js` | **The System Design application** — extracted from `system-design.html` by `tools/split-system-design.py` into ordered slices that share global scope (the same shape as `js/app/*.js`). Cut along the code's own section banners, not by line count: state+Leitner · diagrams · tags · plans · topic landing · topic home · unit detail · component catalog · session · question render · share · summary · stats · keyboard · routing+shell. **Load order matters** and is gated by `--check`. The page was 2,341 lines of which only ~94 were markup, and it was touched in 22 of the last 43 product commits — more than any other file in the repo. |
| `tools/split-system-design.py` | The one-shot extraction above (concatenation asserted byte-identical to the block it replaced). `--check` survives it and guards what outlives the move: the `<script>` tags must stay in split order, every declared slice must exist, and `js/sd/` may hold nothing the page doesn't load. Registered in `check-all.js`. |
| `ds/shell.js` | **The app shell** (D15 phase 2) — the primary nav and the header (breadcrumb · scoped progress · what's due), mounted by **both** `index.html` and `system-design.html`. A destination declares a **route**, never a DOM id; the active highlight derives from the `parent` chain; anything page-local (palette, Settings, Progress) is a **capability the page passes in**, and one it can't service simply isn't rendered. This replaced `js/app/16-ds-nav.js`, whose items were ids it synth-clicked — six of which existed only on index.html — which is why System Design had no rail and no header. |
| `js/app/16-ds-shell.js` | The main app's shell adapter: capabilities, the scope→stats resolver, the due count, and the crumb title/section resolvers. Absorbed the former `js/app/25-breadcrumb.js`. `system-design.html`'s mirror-image adapter is in `js/sd/15-routing-shell.js`. |
