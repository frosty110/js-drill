...
| `tools/cdp/home-nav.js` | Durable probe for the whole navigation flow — boot policy, track cards, subcategory expansion, Continue-vs-Review semantics, scoped review + HUD geometry, deep links, System Design routes. 59 assertions across mobile + desktop. |
...
| `system-design.html` | Standalone **System Design** memorization drill (conceptual recall, no code execution). **Multi-topic**: a topic landing → per-topic chapter list → drill. Topics: **DDIA** (12 ch), **The System Design Interview** (framework + estimation, 7 §), **Building Blocks** (components, 7 §), **Canonical Design Problems** (17 worked problems). Two question types: **MC** and **open** ("explain & apply" — reveal model answer + self-grade). A topic manifest can set `ordered:true` (design problems) so its units drill in authored arc order instead of due-first shuffle. Tapping a unit opens a **detail screen** surfacing the authored `keyTakeaways` as a "Key Ideas" panel + a "Drill all" and (where questions carry `crux:true`) a "Crux only" fast-drill of just the signature questions. The detail screen also renders a **visual study set**: `data/system-design/infographic-plan.json` plans distinct graphics per lesson; `infographic-sets.json` authors each sheet's description, numbered flow, numbers, priorities, and trade-offs before pixels. Canonical Design Problems use multi-image sets (all 17, 3–5 sheets each); Building Blocks and DDIA keep a single illustrated sheet apart from the `c05`/`ch05` pilots — 88 PNGs total. The in-page **Final Interview Whiteboard** widget is gone: for design problems its four panels became the lesson's multi-sheet set (one full portrait sheet per moment instead of four columns on one raster). `drill-infographic-set` renders the ordered lesson set; `drill-infographic` and one shared workspace provide full-screen Fit/100%/zoom/pan/pinch/download. **Diagrams**: content may use legacy singular `diagram` or a `diagrams[]` visual deck. Each deck item is `{id,title,kind,role,takeaway,afterQuestion,code}`. `afterQuestion` attaches each focused visual to the relevant revealed answer. Every canonical design problem deck contains four architecture diagrams (overview, signature mechanism, scale/path, failure/consistency); focused question-level `diagram` request flows remain supported. Mermaid loads from CDN and degrades gracefully (shows source) if unavailable. Reuses `ds/tokens.css` + `ds/components.css` + `js/storage.js` (design-loop P8 family unification); Leitner SR under `jsdrill.systemdesign.v1`. Linked from the main app topbar **and deep-linkable**: `#/<topic>`, `#/<topic>/<unit>`, `#/<topic>/mixed` (due-first review) — each screen publishes its route with `replaceState`, and an external hash change re-renders (`parseRoute`/`applyRoute`). Every manifest chapter entry carries a `questions` count (validator-gated) so the main app's Home can show System Design progress from 5 small fetches instead of all 43 unit files. Content in `data/system-design/<topic>/`. |
| `assets/system-design/infographics/` | 88 committed static PNGs under `components/`, `ddia/`, `design-problems/`. Two forms coexist: a converted lesson uses `<topic>/<lesson>/<graphic>.png` (all 17 design problems + the `c05`/`ch05` pilots); an unconverted one keeps `<topic>/<lesson>.png`. `infographic-plan.json` records the eventual target for every lesson, so it stays the conversion roadmap; see `INFOGRAPHIC-AUTHORING.md` for the quality bar a generated set must clear first. |
| `tools/build-system-design-infographic-sets.js` | Rebuilds compiler-authored study copy from reviewed lesson takeaways, diagram captions, explanations, and interview rubrics while preserving the ten hand-authored pilot sheets. |
| `tools/generate-system-design-infographic-sets.js` | Deterministically renders compiler-authored study guides with reusable architecture primitives and exact text; requires Inkscape + ImageMagick. Authoring guidance is in `data/system-design/INFOGRAPHIC-AUTHORING.md`. |
...
| `tools/generate-system-design-infographics.js` | Deterministically renders legacy single-image lessons and deliberately skips/preserves lessons registered in `infographic-sets.json`; requires Inkscape + ImageMagick. Authoring guidance is in `data/system-design/INFOGRAPHIC-AUTHORING.md`. |
...
| `tools/validate-system-design.js` | Structural validator across all system-design topics (topics↔manifest↔disk parity, part coverage **exactly once**, authored tags against `tags.json`, question/diagram/spec schema, complete 36-lesson infographic plan, authored set schema/count/order, and exact registered PNG paths/dimensions). A plan entry may set `"pending": true` to declare artwork not yet landed — its PNG check is skipped and counted (`… N pending artwork`) instead of failing, so lesson content can land green while hand-drawn sheets are produced out-of-band. |
| `tools/cdp/sd-tags-nav.js` | Durable probe for the design-problems taxonomy + faceted filter — 7 mechanism families, contiguous `displayNum` over non-sequential ids, chip rows, AND/OR filter semantics, empty state, tag deep links, other-topic isolation, mobile+desktop overflow. 35 assertions. |
