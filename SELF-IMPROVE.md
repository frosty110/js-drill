...
## Next iteration
- **Suggested mode:** ship — roadmap entry #4 (iter-31's L1 Rapid-Fire Drill). Single-iter scope; mobile-native; ZERO new content authoring; no PROFILE-amendment dependency; cites PROFILE.md line 31 verbatim ("L1 is the smoothest interaction on mobile — highest throughput for the mobile user"). First entry designed under the iter-31 meta-finding's "recombine existing data into a new mobile-first surface" constraint. Fourth adversary test under the new architecture.
- **Signal pointing there:** Iter 31 vision (forced by 3-pivot streak) produced 3 new roadmap entries with the iter-30 adversary's hard constraints baked in: (1) no per-lesson content authoring required; (2) no governance-blocked PROFILE amendment dependency; (3) survives the audit's "measured pain alternative" challenge. Both vision subagents independently converged on the same 3 buckets: cross-lesson L1 stream, Reference-card cloze/flash, per-lesson temporal sparkline. Iter-31's diagnostic of the 3-pivot pattern (loop over-indexes on "ambitious new modality" features when shippable gradient is "recombine existing data into new mobile-first surface") explicitly informs the entry design. The iter-26 entries (Recognize, Post-Mortem, Audio) are now formally marked BLOCKED in roadmap.md — they remain queued but require a frame iter for PROFILE-amendment ratification before re-attempt. Rapid-Fire Drill is the lightest of the new 3 (single-iter, single-button addition to existing control row, runtime-only data); first non-content ship since iter 24 if it lands.
- **Veto condition:** skip Rapid-Fire ship if (a) user prefers Reference Flash (roadmap #5 — also single-iter, also zero authoring, tests active-recall on Reference tab — different surface but equally shippable); (b) user prefers a frame iter to ratify PROFILE amendments A/B/C and UNBLOCK the iter-26 entries (would require user override of the 2-per-10 frame ceiling); (c) user wants an audit iter (`/lesson-audit --changed-only` on the lessons touched in iters 28+29+30) to measure whether the rewrites actually moved the means as predicted — would inform whether more content work is worth it.
...
*(iters 1–20 trimmed to keep the log at 10 entries — see git history:
