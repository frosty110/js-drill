...
## Next iteration
- **Suggested mode:** ship (roadmap-tagged multi-iter wire/ship) — Sparkline iter 2 of 2 (`[product/feature-wire]` or `[product/feature-ship]` per CLAUDE.md). Iter-32 shipped the SCAFFOLD (schema `__v: 6`, `state.history`, `appendHistory()` helper, 4 hook sites in `markPassed`/`recordWrong`, flag-gated `renderSparkline()`). Iter-33 should: (1) wire the sparkline into `updateLessonHeaderInPlace` / `renderLesson` so it shows in the lesson header by default; (2) polish CSS for the tick layout; (3) add mobile CDP probe `tools/cdp/sparkline-renders.js` (mandatory per SKILL.md for UI changes); (4) flip `window.__sparklineEnabled` default to `true`. Theme map per `docs/learning-strategies/spaced-repetition.md` updated iter-32; iter-33 confirms the doc → UI link by surfacing the data.
- **Signal pointing there:** Iter-32 was scaffold-only (compiled to `[product/feature-scaffold]` per CLAUDE.md multi-iter subtypes). The roadmap entry #6 estimate was "2-iter scaffold+ship" — iter-33 closes the feature. Adversary's pivot rationale from iter-32 (success-criterion must be learning-outcome not adoption-metric) is honored by iter-33's mobile-probe validation: probe asserts the sparkline RENDERS for a lesson with seeded history events, NOT that users adopt the feature. Real adoption measurement is post-ship via the existing `state.history` cap eviction signal (if HISTORY_MAX hits frequently, the user IS drilling that lesson repeatedly).
- **Veto condition:** skip Sparkline iter-33 ship if (a) the iter-32 scaffold has any bugs surfaced by browser-test (history-append not firing in some sites, schema migration error on existing localStorage); (b) user prefers Rapid-Fire Drill (deferred from iter-32 PIVOT — would require fixing audit theme #3/#4/#7 absurd-distractor floor first per iter-32 adversary); (c) user wants theme-7 part 2 (the ~20 remaining absurd-distractor edits — also the adversary's Alt B from iter-32, ships as Rapid-Fire prerequisite).
