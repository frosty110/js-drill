...
## Next iteration
- **Suggested mode:** ship — Reference-Card Flash (roadmap entry iter-31 #5) — the last untouched iter-31 entry. Single-iter ship; no authoring; runtime cloze tokenization of `reference.code` via CodeMirror tokenizer (already loaded). Closes the iter-31 roadmap (entry #4 Rapid-Fire deferred per iter-32 adversary, #6 Sparkline shipped iters 32+33, #5 Flash is the remaining one). OR ship: theme-7 part 2 (~20 remaining absurd-distractor L1 edits — Rapid-Fire prerequisite).
- **Signal pointing there:** Iter-34 completed the sparkline ship with mobile probe — feature now fully validated. Pivot rate 4/7 = 57%. Reference Flash is the natural next iter-31 entry; tests whether the meta-finding hypothesis (recombine existing data into mobile-first surface) reproduces for a different surface (active recall on Reference tab via cloze blurs).
- **Veto condition:** skip Flash ship if (a) user wants audit iter (`/lesson-audit --changed-only` on lessons touched iters 28-33 to measure impact); (b) user wants theme-7 part 2 (the ~20 remaining distractors — Rapid-Fire prerequisite); (c) frame iter for PROFILE-amendment ratification.
