...
## Next iteration
- **Suggested mode:** ship — follow-up #1: `tools/cdp/sparkline-renders.js` mobile probe (deferred from iter-33 due to context constraints — SKILL.md mandates mobile probe for UI changes). Single-iter, ~50 LOC probe seeding `state.history` with synthetic events, navigating to a lesson, asserting sparkline DOM ticks render with expected colors at iPhone viewport. OR follow-up #2: theme-7 part 2 (the ~20 remaining absurd-distractor L1 edits — Rapid-Fire prerequisite per iter-32 adversary, ~30 min mechanical work).
- **Signal pointing there:** Iter-33 sparkline ship landed REBUT — first non-pivot in 5 ship iters (pivot rate 4/6 = 67%). Feature surface is live for all users (`__sparklineEnabled = true` default). Mobile probe is the one remaining SKILL.md requirement; safe to do as a small follow-up ship since the feature itself is in production with the validator passing.
- **Veto condition:** skip if user wants (a) instead an audit iter (`/lesson-audit --changed-only` on the 10 lessons touched in iters 28-30 to measure rewrite impact); (b) Reference-Card Flash (roadmap entry iter-31 #5 — the other untouched iter-31 entry, also no-authoring, also recombines existing data); (c) frame iter to ratify PROFILE amendments A/B/C from iter 26 (would unblock the 3 BLOCKED roadmap entries — needs user override of 2-per-10 frame ceiling).
