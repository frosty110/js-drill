...
The catalog holds every answer but could only be *read*, and PROFILE.md is
explicit that reading is the weakest form of study. So the same data is
projected into a recall drill: **308 cards across 11 decks**, one deck per
category, generated by `tools/build-component-cards.js` into the
`component-cards` topic.

They are emitted as ordinary `open` questions — prompt → reveal → self-grade —
which is the flash-card shape the drill already ran. Nothing new in the runner.
Your "simple answer / detailed answer" split maps onto the existing fields:
`points[]` is the rubric checklist, `answer` is the prose.

| Template | Asks | Built from |
|---|---|---|
| **selection** | given these signals, which block? | `reachFor` |
| **rejection** | when is it wrong, and what does it cost anyway? | `avoid` + `costs` |
| **failure** | how does it break at 10x? | `failureModes` |
| **discrimination** | this vs its siblings — what decides? | `alternatives` |
| **transfer** | what is it doing across the canonical problems? | the edge annotations |

**Derived, never authored** — so a card can never drift from the component it
tests, and adding a component yields its cards for free.

### Two rules the generator holds

**1. The order ledger.** A card's SR key is `topic/unit/index` and its share
code is positional, so *position is identity*. Sorting by anything derived from
the source would mean that adding one edge from `caching` to an earlier problem
inserts a card mid-deck and silently repoints every user's Leitner state —
invariant 1's failure arriving through the back door. `order.lock.json` assigns
each card a permanent slot on first sight and never moves it; new cards append,
retired cards leave their hole open.

**2. No padded rubrics.** A card is emitted only when the source supplies ≥3
real rubric points. 17 do not — mostly `transfer` for components used in fewer
than three problems, where "name the one place this appears" is a lookup rather
than retrieval practice. Those are **reported by the generator, not invented**;
padding a rubric to clear a threshold is exactly how the auto-derived filler
that the validator's FILLER guard now blocks came to exist.

The generator also reports the 5 components whose *domain noun is their name*
(Work Queue, Tenant Isolation, …), where the selection prompt cannot avoid
naming its own answer. An easier card, not a broken one — and knowing which
they are is what would drive a `reachFor` rewrite later.
