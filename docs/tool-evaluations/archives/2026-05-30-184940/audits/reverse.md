...
- 2026-05-30 Salvage edits 1+2 applied (Phase 5 band-promotion). (1) Per-distractor "why wrong" — `_reverseBuildDeck` now stores `distractorOutputs` map (promptMasked → output) per card at `js/app/07-drills-swap-speedrun.js:599-621`; reveal handler at `:706-715` surfaces "Your pick would have produced `<output>`" when distinguishable (+1 Feedback). New `.reverse-distractor` rose-tinted style at `css/04-drills.css:799-808`. (2) Hold-but-reset-dueAt SR write on win — guarded to mastered+due at `:670-681` mirroring Predict/Claim/Gotcha pattern (+1 Spacing). Projected 17→19 (KEEP ship-quality). Validator: 895 passed, 0 failed.
