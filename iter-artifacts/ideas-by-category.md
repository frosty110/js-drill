...
- ~~**PWA install + offline drilling**~~ → **FULLY SHIPPED** across iter 113 (SW cache half) + iter 145 (manifest.webmanifest + icon.svg + icon-maskable.svg + 7 `<head>` link/meta tags + #install-btn + beforeinstallprompt/appinstalled handler). Push API remains a natural future follow-on but not blocked.
...
4. **PWA install + offline drilling** *(Cat 5)* — → SHIPPED iter 145 as 📲 PWA install (manifest.webmanifest + icon.svg + icon-maskable.svg + 7 `<head>` tags + #install-btn + beforeinstallprompt/appinstalled handler). Pairs with iter-113 SW cache. **Shortlist now fully drained: items #1 → iter 108 🍀 Lucky; #2 → iter 144 📝 Notes→Code; #3 → already-shipped iter-117/118 §9A; #4 → iter 145 📲 PWA install; #5 → iter 109 🔖 Match. The iter-93 shortlist had a 37-iter lifespan across 5 shipped items + supported 6 vision iters.**
