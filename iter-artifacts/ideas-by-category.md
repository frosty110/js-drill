...
**Cross-cutting concerns (iter 38 add):**
- **Interview-life integration gap:** the catalog and the app today treat interview prep as a *solo, decontextualized activity*. Every existing surface assumes the user is mid-drilling with no calendar context, no real-world interview history, no behavioral round to also prepare for. The five new entries above (pre-interview ramp, post-interview gap log, STAR generator, "I'm stuck" simulator, mock-kickoff ritual) collectively introduce *event-embedded* prep. Track whether the loop ships any of these in the next 6 iters; persistent zero would indicate the app is structurally biased toward perpetual-prep over scheduled-prep — a misalignment with PROFILE.md's implied use case (rusty engineer is rusting *toward an interview*, not in eternity).
...
- **[iter 36] Debugging vs. solving direction:** entries like "Where's your bug?", "Smell test", "Anti-templating", and the Crystal Ball entry in Cat 1 collectively introduce a *debugging-and-evaluating* direction that the L1/L2/L3 ladder doesn't drill. Real coding interviews split time roughly evenly between "produce a solution" and "evaluate / debug / explain a solution" — the catalog should track whether the loop ships any debugging-direction surface in the next 6 iters.
...
- **Prunes:** 3 entries removed outright.
  - *Vibration-cue Morse drill* (Cat 8, iter 36) — self-flagged "probably a dead-end" at authoring. Honoring the flag.
  - *"Last 5 lessons drilled" recents* (Cat 5, pre-iter-35) — substantively covered by existing `lastLessonId` state + the Session resume feature shipped earlier.
  - *Reverse-SR (forced re-derivation)* (Cat 3, iter 36) — folded into the Cat-9 Re-derive-without-Reference entry as a "2-option ship" sub-clause; same shape, different trigger.
...
- **Catalog structural observation:** Cat 9 (Interview Conditioning) now has 24+ entries and is becoming hard to scan. Flag for iter-41+ reorganization candidate: split into "Behaviors" (iter-35 batch — clarifying questions, hot-seat, narrate) and "Methodology" (iter-36–38 batches — code-reading, debugging, integration). NOT executed this iter (bigger edit than appropriate for a discipline iter).
- **Iter-41 outlook:** if the loop fires again, the right move is most likely (a) execute the Cat-9 split flagged above, (b) explicit "no meaningful action this iter" if pruning + reorg both feel forced, or (c) actual roadmap.md promotion if the user signals they want this loop to act on that authority. Continuing to find prune candidates iter after iter would itself signal the catalog has stabilized and the loop should stop. **Trajectory check:** iter-35→iter-40 deltas went +269, +30, +45, +15, +10, **net −3** — strict monotonic decay, ending in negative. The loop is converging on a stable steady-state catalog. Two more cap-binding iters and the right answer is to stop the loop.
