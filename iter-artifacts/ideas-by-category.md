...
- **"What's missing?" critical-line fill** — **SHIPPED iter 41 (MVP).** 🎯 button on L3 tab; pre-fills editor with canonical scaffold and replaces lines marked `L3.criticalLines: [int]` with `/* ___ FILL LINE N ___ */` markers. User types just the 1-2 load-bearing lines (the algorithm's *insight*). Same Run handler validates. Hint tier — no SR demote. **3 lessons authored as MVP:** two-sum [6,8] (complement + check-before-set), valid-parentheses [8,14] (pop-and-compare + final-empty check), p-anagrams [6] (sorted-key fingerprint). Validator bounds-checks `criticalLines` against canonical line count + asserts non-empty + non-comment lines. Mobile probe `tools/cdp/critical-lines.js` (5 assertions). **Follow-up iters:** expand authored set to 10-20 high-traffic patterns.
