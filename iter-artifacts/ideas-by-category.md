...
- **Trace-Hop (pick-the-middle-state recall)** — see [`roadmap.md` iter-90 entry #2](roadmap.md). **SHIPPED iter 93.** First surface drilling **positional state recall** over `walkthrough.trace` yields. 🧬 Trace-Hop sidebar button → 8-card mobile session; each card shows 3 consecutive trace frames (K-1, K, K+1) with the middle frame's state BLANKED + 4 same-trace state options; distractors exclude K-1/K+1 to prevent trivially-wrong giveaways. Tap reveals ✓/✗ + lesson + drill CTA; misses route to state.weakness. Distinct from 🪲 Bug-Hunt's anomaly-detection direction — Trace-Hop tests "given the line that ran, which locals snapshot is correct" (the mental model needed to write canonical from scratch). Zero per-lesson authoring — every walkthrough-bearing lesson (99/99) inherits the surface for free. Mobile probe 5/5 PASS.
- **Notes Cloze Tap-Drill (note-keyword cloze with distractors)** — see [`roadmap.md` iter-95 entry #1](roadmap.md). **SHIPPED iter 97.** Third recall direction over `reference.notes[]` corpus. 📝 Notes sidebar pill → 12-card mobile session showing one note with one keyword blanked + 4 same-section MC distractors. Keyword-blanking heuristic: last eligible token (≥4 chars + not in 50-stop-word filter + has at least one letter). Distinct from 🎰 Gotcha (whole-note yes/no recognition) and 🃏 Flash (code-token cloze) — Notes Cloze forces actual keyword recall with MC pressure, closest to interview pressure on note content. Mobile probe 5/5 PASS.
...
- **🪐 Mechanic Constellation Quiz** — see [`roadmap.md` iter-95 entry #2](roadmap.md). **SHIPPED iter 98.** First surface drilling `mechanics[]` as a recall TARGET (Bridge/Matrix/modal all USE mechanics as input). 🪐 sidebar pill → 10-card mobile session showing 1 mechanic + 6 lessons (3 tagged + 3 not, same-section preference); 3-tap multi-select with per-tap feedback. Misses (wrong-tap on lesson) route to state.weakness. Mobile probe 5/5 PASS.
- **🧠 Mechanic-Bridge (cross-track transfer routing)** — see [`roadmap.md` iter-90 entry #3](roadmap.md). **SHIPPED iter 94.** Converts the iter-63 Mechanics × Track Matrix's diagnostic ⚠ marker into a 1-tap routing action. 🧠 Bridge sidebar pill (fuchsia, mechanic-family color) auto-shows when transfer gaps exist (mechanic mastered in one track but unmastered in another); tap routes to the target's L1 with a 2.2-sec fuchsia toast prefacing the source lesson + mechanic label. Lazy-loads MECHANIC_INDEX on first updateReviewBadge call. Pure derivation from MECHANIC_INDEX × state.progress × manifest.track; no schema bump. Closes the iter-63 "diagnostic-but-not-actionable" gap on PROFILE.md §What they need #2 (pattern fluency / produce without thinking). Mobile probe 5/5 PASS.
...
- ~~**"Smell test" complexity-claim drill**~~ — **SHIPPED iter 79** as 📐 Claim. Sidebar button → 5-card session reading from `data/complexity-claims.json` (10 curated high-traffic patterns: two-sum, valid-palindrome, p-bfs, binary-search, p-3sum, p-merge-intervals, p-min-window, valid-parentheses, p-contains-dup, p-anagrams). Each card shows canonical + a randomly-chosen claim (50/50 actual vs distractor) + 2-tap correct/wrong; reveal shows actual + a one-line note (why the claim was right/wrong). Misses route to `state.weakness`. Schema-additive `state.claim = {attempts, correct, sessions, lastRunAt}`. ~160 LOC JS + 35 CSS. **Expandable** by appending entries to the JSON registry — no code change needed.
- ~~**Edge case pre-enumeration drill**~~ — **SHIPPED iter 81** as 🛡 chip strip on L3 (MVP scope). 6 fixed toggle chips (empty / single element / duplicates / max size / negative / no solution) rendered above the editor on every L3 tab; tap-toggles a "considered" state per chip. Hidden during Mock Interview (mock is no-scaffolding by design). Pure UX nudge — no scoring, no per-lesson curation. **Future graduation:** ship `data/edge-cases.json` mapping each lesson to `{handled: [], unhandled: []}` and reveal a comparison after L3 pass (what user considered vs what canonical actually handles). Catalog parking-lot entry retained here as a possible second-tier ship.
