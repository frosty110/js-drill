...
- **Reverse-SR (forced re-derivation)** — at the LONGEST SR interval (30d), instead of just resurfacing the lesson, force a *blank-editor recall* (skip the Reference read). If the user passes from cold storage, the bucket extends to 60d+; if they fail, demote sharply. The "true mastery" tier the current SR ceiling doesn't enforce.
...
- **Vibration-cue Morse drill (experimental)** — for the eyes-free walking case, app vibrates Morse-encoded L1 question outline (subject + 4 distractors). User taps to acknowledge each pulse. Probably a dead-end but worth one ship attempt because if it works it unlocks a modality nothing else can touch. Cheap to prototype.
...
- **Iter-40 outlook:** the loop will fire again in ~30 min on the same prompt. The right move next iter is likely (a) prune low-leverage parking-lot entries to free headroom OR (b) report explicit "no meaningful add this iter" if the cap genuinely binds. Continuing to find new "bridges-a-cross-cutting-gap" entries iter after iter would itself be a sign the bar is too low.
