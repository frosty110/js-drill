...
**Status:** Phase 2 SHIPPED iter 127 (shell + dropdown machinery + mobile media query). Phase 3 SHIPPED iter 128 (real `.topbar-item` rows in 5 menus via DRY taxonomy + actionability filter + delegated synth-click handler — 53/53 mobile probe PASS). Phase 4 (iter 129) removes the now-redundant ~30 sidebar mode-launcher buttons + extends Cmd-K index to topbar-items. Optional Phase 5 (iter 130) ships the mobile ☰-Menu sheet that exposes the 4 categories vertically. (Phase 1 of the epic = user's pre-iter framing — not a code phase.)
