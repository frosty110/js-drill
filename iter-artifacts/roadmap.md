...
**Status:** ✅ **4-PHASE EPIC COMPLETE.** Phase 2 SHIPPED iter 127 (shell + dropdown machinery + mobile media query). Phase 3 SHIPPED iter 128 (real `.topbar-item` rows in 5 menus via DRY taxonomy + actionability filter + delegated synth-click handler — 53/53 mobile probe PASS). Phase 4 SHIPPED iter 129 (visual hide via CSS `display: none !important` on 35 button IDs — **pivoted from "DELETE" to "HIDE"** in Step 2 once the load-bearing concern surfaced that the topbar synth-click bridge + ~12 existing CDP probes all query the buttons by ID; buttons stay in DOM, structural dependencies preserved, user sees clean sidebar with just lesson list + progress + state-indicator badges — 66/66 mobile probe PASS + regression spot-check on cheatsheet-pdf 20/20 + gauntlet 19/19 + l3-chips 20/20). Cmd-K palette never needed extension — its existing `aside button[id]` query continues to find hidden buttons. **Optional Phase 5** (iter 130+, deferred): mobile ☰-Menu sheet that exposes the 4 categories vertically on mobile (where `.topbar-menus` is currently `display: none`). Defer Phase 5 unless mobile users report friction reaching modes via Cmd-K alone. (Phase 1 of the epic = user's pre-iter framing — not a code phase.)
