...
**Status:** SHIPPED iter 147. New sidebar 📞 Phone Screen button (cyan-300 hover) → chained 3-card session under ONE unbroken timer. Card 1 = syntax-track Reference-readonly warmup with notes; card 2 = pattern-track L3 (CodeMirror editor + Run/Give-up; pass advances on `===` match against expectedOutput); card 3 = L2 fill-blanks on a mechanic-overlap lesson (chosen via existing MECHANIC_INDEX × `_bugHuntShuffle` over card-2's mechanics; fallback to any non-card-2 pattern when no overlap). New `_phoneScreenBuildDeck()` + `startPhoneScreenSession()` (~330 LOC JS + ~125 CSS) reusing Mock's timer pattern + recognize-shell layout. Schema-additive `state.phoneScreen = {sessions, completions, lastRunAt}`. Mobile probe `tools/cdp/phone-screen.js` **30/30 PASS** at iPhone viewport including the load-bearing timer-continuity assertion (T at end of card 1 = 0:01; T' at start of card 2 = 0:02 — clock kept ticking across transition). Empirical-scan-first pattern (now 8-time-validated, iter-99/122/140/141/142/144/145/147) caught (a) 72 of 79 pattern lessons have ≥1 mechanic + 24 mechanics shared by ≥3 lessons (overlap pick reliable), (b) MECHANIC_INDEX requires `ensureMechanicIndex()` async init, (c) Mock timer infrastructure is reusable directly. Validator 698/0. Step 1A trace: § Next nominated this directly from iter-146 roadmap entry #1.
...
**Status:** SHIPPED iter 142. **iter-139 vision queue FULLY DRAINED** across iter 140 Pace-Bar + iter 141 Haptic Tap-Pulse + iter 142 Mutate-and-Predict (3 of 3 promotes shipped in 3 consecutive iters — fastest queue-to-drain cadence on a 3-promote vision iter). New sidebar 🔀 Mutate button → 5-card session; canonical with ONE line mutated; 4 MC consequence-class taxonomy (Output unchanged / Wrong output same shape / Runtime error / Different output type). Reuses iter-73 Bug-Hunt's `BUG_HUNT_MUTATORS` + `_bugHuntCollectMatches` + `_bugHuntLineOf` + `_bugHuntShuffle` helpers — operator-mutation infrastructure stays in one place. New helpers `_mutateInferType` + `_mutateClassify` + `_mutateClassifyMutation` + `_mutateBuildDeck` + `startMutateSession`. **Hang-protection finding (load-bearing — first revealed by iter-142 probe hang on initial run):** 3 of 10 BUG_HUNT_MUTATORS (`++ → --`, `-- → ++`, `&& → ||`) can produce sync infinite loops when applied at loop counters / restrictive guards. `runCode` runs `new Function(...)(fakeConsole)` synchronously — Promise.race timeout can't interrupt it; the only safe protection is not picking those mutations. Mutate uses a curated 7-mutator safe subset (`MUTATE_MUTATORS = BUG_HUNT_MUTATORS.filter(...)`). Bug-Hunt remains on the full 10-mutator pool but the hang risk is now documented for a future iter to address. ~270 LOC JS + ~22 LOC CSS. Validator 698/0. Mobile probe `tools/cdp/mutate-predict.js` **30/30 PASS** at iPhone viewport across 7 phases including the load-bearing differentiator assertion (correctClass ∈ {0,1,2,3} taxonomy, NOT buggyLine localization). Reused .recognize-shell + .whatif-shell + .recognize-opt CSS from iter-49/122/etc (minor new styles only: `.mutate-mutator-tag`, `.mutate-mutator` (orange-400 monospace chip), `.mutate-observed`). Step 1A trace: § Next nominated this ship directly from this roadmap entry.
...
**Status:** ✅ **5-PHASE EPIC FULLY COMPLETE.** Phase 2 SHIPPED iter 127 (shell + dropdown machinery + mobile media query). Phase 3 SHIPPED iter 128 (real `.topbar-item` rows in 5 menus via DRY taxonomy + actionability filter + delegated synth-click handler — 53/53 mobile probe PASS). Phase 4 SHIPPED iter 129 (visual hide via CSS `display: none !important` on 35 button IDs — **pivoted from "DELETE" to "HIDE"** in Step 2 once the load-bearing concern surfaced; buttons stay in DOM, structural dependencies preserved, user sees clean sidebar with just lesson list + progress + state-indicator badges — 66/66 mobile probe PASS + regression spot-check on cheatsheet-pdf 20/20 + gauntlet 19/19 + l3-chips 20/20). Phase 5 SHIPPED iter 130 (mobile-only 📂 Browse button at ≤767px; tap opens dropdown with 4 category-picker rows; tap a category re-renders dropdown in place with that category's items; tap an item fires existing iter-128 synth-click bridge through iter-129's hidden sidebar button — 82/82 mobile probe PASS including the end-to-end "tap Gauntlet item opens Gauntlet picker shell" assertion that mechanically proves all 3 architectural layers fire together). Cmd-K palette never needed extension. (Phase 1 of the epic = user's pre-iter framing — not a code phase.)
...
**Status:** SHIPPED iter 126 (see [SELF-IMPROVE.md § iteration log iter 126](../SELF-IMPROVE.md)). 📱 Save button added to Cheatsheet modal toolbar; new `_buildPrintableCheatsheetHtml()` emits 214 KB HTML doc covering all 154 lessons across 29 sections with embedded print stylesheet; `exportCheatsheetToPdf()` opens window SYNC-from-click-handler (iOS popup-allow), writes HTML, fires `win.print()` after 600ms layout delay. Plain code blocks (no syntax highlighting) — chosen deliberately for PDF reading + cross-renderer compatibility. Mobile probe `tools/cdp/cheatsheet-pdf.js` **20/20 PASS** via a new `window.open` shim pattern (preserves function-under-test while CDP verifies the structural + behavioral invariants the native print dialog hides). **First Cat 6 surface that produces a user-owned native-OS file** — closes the leaves-the-app-entirely gap that Offline Pack iter 113 (in-app offline) didn't address. **iter-124 queue FULLY DRAINED across iter 125 + 126 — roadmap.md Queued is now EMPTY.**
...
**Status:** SHIPPED iter 122. **iter-120 queue FULLY DRAINED** (Cinema iter 121 + What-If iter 122, 2 of 2 across 2 iters — fastest queue-to-drain cadence yet at this 2-promote vision-iter shape). Sidebar 🧪 What-If pill (violet-300 hover, distinct from Crystal purple-300) opens an 8-card mobile session. Each card shows the (memorized) canonical + ONE walkthrough-example input phrase + 4 MC options for the function's output. Correct = that example's `expected`; 3 distractors = the OTHER 2 examples' `expected` outputs (intra-lesson plausible-wrongs, same-output-type by construction) + 1 algorithmic shift on correct (±1 numeric / flipped boolean / reversed-or-swapped array / `[]`). Misses route to `state.weakness` like every drill. Schema-additive `state.whatif = {attempts, correct, sessions, lastRunAt}` — no `__v` bump. ~270 LOC JS + ~80 CSS. Mobile probe `tools/cdp/whatif.js` **20/20 PASS** at iPhone viewport (390x844) across 5 phases including both correct + wrong branches verified within deck loop. Validator 698/0.

**Adapted spec — roadmap entry's "fresh inputs" framing revised post-scan.** Empirical corpus scan (iter-122 setup) found ALL 99 Patterns/Applied lessons encode inputs in `walkthrough.example.label` as a free-form string with `→` separator (e.g. `[2,7,11,15] target=9 → [0,1]`); NO structured `inputs` field exists; output types are highly uniform (162 strings + 99 numbers + 33 booleans across 297 examples). Adapted v1 to use the 3 walkthrough examples as the input pool — per-lesson recall task — rather than synthesizing fresh inputs. **Cognitive operation:** for THIS specific input, what does the canonical output? — distinct from Crystal Ball iter 77 ("what's the typical output of this canonical?") and Walkthrough Quiz iter 36 ("what state comes next in the trace?"). Third surface in the "predict the output" family covering end-state per specific input. v2 candidate (deferred): synthesize fresh inputs by mutating label's input portion — requires per-pattern label parser. **Same adapted-spec pattern as iter-99 Reverse-Walk** where the entry assumed "many <4 examples" but corpus had exactly 3. Pattern now 2-time-validated: when a roadmap entry describes data dependencies, the first iter action is an empirical corpus scan, not trusting the entry's data-shape assumptions.
