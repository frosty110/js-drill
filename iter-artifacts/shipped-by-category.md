...
- **🗂 Notes→Lesson Reverse Lookup (cross-corpus localization)** *(iter 102)* — sidebar 🗂 Locate → 10-card mobile session. Each card shows ONE `reference.notes[i]` string (≥30 chars uniqueness proxy) + 4 lesson-title MC buttons with section sublabel (1 correct + 3 same-section-preferred distractors per iter-97 Notes Cloze pattern). Tap reveals correct lesson + section + drill CTA. Misses → `state.weakness`. **Third recall direction over `reference.notes[]`** — Gotcha = whole-note yes/no recognition; Notes Cloze = intra-note keyword cloze; Locate = note → which lesson localization. Corpus is now fully triple-mined. Schema-additive `state.notesLocate`. ~210 LOC JS + ~75 CSS. Mobile probe `tools/cdp/notes-locate.js` (5 assertions).
